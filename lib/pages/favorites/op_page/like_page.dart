...
class LikePage extends StatefulWidget {
  final List<ActionDatum> likeList;
  const LikePage({super.key, required this.likeList});
...
class _LikePageState extends State<LikePage> {
  ...
  void initState() {
    ...
    if (opVm.likeList.isEmpty) {
      opVm.getLikeList(false);
    }
  }
  ...
  Widget build(BuildContext context) {
    return SmartRefreshWidget(
        ...
        onLoading: () async {
          // 上拉加载
          if (await opVm.getLikeList(true)) {
            refreshController.loadComplete();
          } else {
        ...
        child: SingleChildScrollView(
          child: Padding(
              ...
              child: Selector<OpVm, List<ActionDatum>>(
                  selector: (context, vm) => vm.likeList,
                  builder: (context, likeList, child) {
                    List<ActionDatum> list =
                        likeList.isEmpty ? widget.likeList : likeList;
                    return Wrap(
